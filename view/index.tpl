<!doctype html>
<html lang="en">
<head>
	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.8.1/css/all.css" integrity="sha384-50oBUHEmvpQ+1lW4y57PTFmhCaXp0ML5d60M1M7uH2+nqUivzIebhndOJK28anvf" crossorigin="anonymous">
	<link href="https://fonts.googleapis.com/css?family=Poppins" rel="stylesheet">
	<link rel="stylesheet" href="{$GET_TEMA}/tema/css/estilo.css">
	<link rel="stylesheet" href="{$GET_TEMA}/tema/css/responsive.css">
	<title>{$GET_NOME}</title>
	<meta name="google-signin-client_id" content="219619071778-4m4a1p3l6pep4bd5ekje465ep4898kt7.apps.googleusercontent.com">
	<script src="https://apis.google.com/js/platform.js" async defer></script>
</head>
<body>
	<header>
		<input type="checkbox" id="bt_menu">
		<label id="lbl_menu" for="bt_menu"><i class="fas fa-bars"></i></label>
		<div class="logo">
			<img src="{$GET_TEMA}/tema/images/logo.png">
			<h2>Pinneapple&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Ofertas</h2>
		</div>
		<nav class="menu" style="width:100%;">
			<ul>
				<li style="width:13%;"><a href="{$GET_SITE_HOME}"><i class="fas fa-home"></i> Inicio</a></li>
				<li style="width:14%;"><a href="{$PAG_CONTATO}"><i class="fas fa-envelope"></i> Contato</a></li>
				
				<li style="width:16%;"><a href="#"><i class="fas fa-tags"></i> Anúncios</a>
					<ul class="ul_cate">
						{foreach from=$CATEGORIAS item=C}
						<li><a href="{$C.cate_link}"><i class="fas fa-tag"></i> {$C.cate_nome}</a></li>
						{/foreach}
					</ul>
				</li>
				<li style="width:15%;"><a href="{$PAG_CARRINHO}"><i class="fas fa-shopping-cart"></i> Carrinho</a></li>
				
				{if $LOGADO == true}
				<li style="width:13%;"><a href="{$PAG_MINHACONTA}"><i class="fas fa-user-circle"></i> {$USER}</a>
					<ul>
						<li><a href="{$PAG_LOGOFF}"><i class="fas fa-sign-out-alt"></i> Sair</a></li>
					</ul>
				{/if}
				
				{if $LOGADO == false}
				<li style="width:13%;"><a href="{$PAG_LOGIN}"><i class="fas fa-user-circle"></i> Conta</a>
					<ul>
						<li><a href="{$PAG_LOGIN}"><i class="fas fa-sign-in-alt"></i> Login</a></li>
						<li><a href="{$PAG_CADASTRO}"><i class="fas fa-user-plus"></i> Cadastro</a></li>
					</ul>
				</li>
				{/if}
				
				<li style="width:25%;" class="busca">
					<form role="search" method="POST" action="">
						<input type="text" class="search" name="txt_buscar" style="width:80%;">
						<button id="btn-pesquisa" type="submit" style="width:18%;"><i class="fas fa-search"></i></button>
					</form>
				</li>
			</ul>
		</nav>
	</header>

	<main style="min-height: 300px;">
		{php}

            Rotas::get_Pagina();
        
        {/php}
	</main>

	<section class="container-fluid newsletter">
		<h2>Inscreva-se e receba informações sobre nossas ofertas!</h2>
		<h4>Ofertas diárias com os melhores descontos, não perca!</h4>
		<div>
			<form role="form" id="Formulario" action="newsletter" method="POST">
				<input type="email" placeholder="Email" id="email_newsletter" required/>
				<button id="bt_newsletter">Inscrever-me!</button>
			</form>
		</div>
	</section>

	<footer>
		<div>Anúncios
			<hr>
			<ul>
				{foreach from=$CATEGORIAS item=C}
				<li><a href="{$C.cate_link}">{$C.cate_nome}</a></li>
				{/foreach}
			</ul>
		</div>
		<div>Suporte
			<hr>
			<ul>
				<li><a href="#">FAQ</a></li>
				<li><a href="#">Reportar Bug</a></li>
				<li><a href="#">Termos e Privacidade</a></li>
				<li><a href="{$GET_SITE_HOME}/adm/login.php">Painel Administrativo</a></li>
			</ul>
		</div>
		<div>Contato
			<hr>
			<ul>
				<li><a href="#">Telefone</a></li>
				<li><a href="{$PAG_CONTATO}">Email</a></li>
				<li><a href="{$PAG_CONTATO}">Quero Anúnciar</a></li>
			</ul>
		</div>
		<div class="center"><i class="far fa-copyright"></i> Todos os direitos a Jonas Boeno - {$TITULO_SITE} - {$DATA} <i class="fas fa-coffee"></i></div>
	</footer>

	<script src="{$GET_TEMA}/tema/js/newsletter.js" type="text/javascript"></script>
</body>
</html>