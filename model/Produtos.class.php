...
Class Produtos extends Conexao{
	 ...
	 private $pro_nome, $pro_fornecedor, $pro_categoria, $pro_ativo, $pro_modelo, $pro_ref, 
    ...
    function GetProdutosFornID($id){
		//query para buscar os produtos de um fornecedor especifico.

		$id = filter_var($id, FILTER_SANITIZE_NUMBER_INT);

		$query = "SELECT * FROM {$this->prefix}produtos p INNER JOIN {$this->prefix}categorias c ON p.pro_categoria = c.cate_id";

		$query .= " AND pro_fornecedor = :id";
	...
	private function GetLista(){
		...
		$this->itens[$i] = array(
			 'pro_id' => $lista['pro_id'],
            'pro_fornecedor'  => $lista['pro_fornecedor'] ,  
	         ...
	         'pro_img'     => Rotas::ImageLink($lista['pro_img'], 480, 480) ,  
	         'pro_img_g'     => Rotas::ImageLink($lista['pro_img'], 700, 700) , 
	         'pro_img_p'     => Rotas::ImageLink($lista['pro_img'], 80, 80) , 
	...
	function Preparar($pro_nome, $pro_fornecedor, $pro_categoria, $pro_ativo, $pro_modelo, $pro_ref, 
            $pro_valor, $pro_estoque, $pro_peso , $pro_altura, $pro_largura, $pro_comprimento ,
            $pro_img, $pro_desc, $pro_slug=null){
        
                $this->setPro_nome($pro_nome);
                $this->setPro_fornecedor($pro_fornecedor);
        ...
        $query = "INSERT INTO {$this->prefix}produtos (pro_nome, pro_fornecedor, pro_categoria, pro_ativo, pro_modelo, pro_ref," ;
        $query.= " pro_valor, pro_estoque, pro_peso , pro_altura, pro_largura, pro_comprimento ,pro_img, pro_desc, pro_slug)";
        $query.= " VALUES ";
        $query.= " ( :pro_nome, :pro_fornecedor, :pro_categoria, :pro_ativo, :pro_modelo, :pro_ref, :pro_valor, :pro_estoque, :pro_peso ,";
        $query.= " :pro_altura, :pro_largura, :pro_comprimento ,:pro_img, :pro_desc, :pro_slug)";
        
        $params = array(
        ':pro_nome'=> $this->getPro_nome(), 
        ':pro_fornecedor'=> $this->getPro_fornecedor(),   
        ...
        $query = " UPDATE {$this->prefix}produtos SET pro_nome=:pro_nome, pro_fornecedor=:pro_fornecedor, pro_categoria=:pro_categoria," ;
    ...
    function getPro_fornecedor() {
        return $this->pro_fornecedor;
    }
    ...
    function setPro_fornecedor($pro_fornecedor) {
