...
if(isset($_POST['forn_nome']) and isset($_POST['forn_email']) and isset($_POST['forn_cnpj'])){
    //variaveis
     ...
     $assunto = 'Você tem um novo cliente!';
     $msg = $smarty->fetch('email_fornecedor_novo.tpl');
     $destinatarios = array(Config::SITE_EMAIL_ADM);
     $email->Enviar($assunto, $msg, $destinatarios);

     echo '<h3 class="alerta pulse">Cadastro Efetuado!!<br> A senha para fazer login foi enviada para o email cadastrado</h3>';

     Rotas::Redirecionar(3, Rotas::pag_FornecedoresADM());

}else{
    $smarty->display('adm_fornecedores_novo.tpl');
